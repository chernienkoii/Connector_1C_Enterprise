...
func (Connector *Connector) ConsumeFromQueueFor1C() {
	...
	go func() {
		for d := range msgs {
			...
			resp, err := http.Post("http://10.1.99.32/REST_test/hs/rabbitmq/rabbitmq_json", "application/json", bytes.NewBuffer(d.Body))
			if err != nil {
				//return err
				Connector.LoggerConn.ErrorLogger.Println(err.Error())
			}
...
func (Connector *Connector) GetAllCustomer(DataBaseType string) (map[string]rootsctuct.Customer_struct, error) {
	...
	case "1C_Enterprise":
		...
		resp, err := http.Get("http://10.1.99.32/REST_test/hs/exchange/custom_json")
		if err != nil {
			Connector.LoggerConn.ErrorLogger.Println(err.Error())
			return nil, err
		}
...
func (Connector *Connector) AddChangeOneRow(DataBaseType string, Customer_struct rootsctuct.Customer_struct, Global_settings rootsctuct.Global_settings) error {
	...
	case "1C_Enterprise":
		...
		//Connector.Global_settings.Enterprise1CAdress
		resp, err := http.Post("http://10.1.99.32/REST_test/hs/exchange/custom_json", "application/json", bytes.NewBuffer(bytesRepresentation))
		if err != nil {
			return err
		}
...
func (Connector *Connector) DeleteOneRow(DataBaseType string, id string, Global_settings rootsctuct.Global_settings) error {
	...
	case "1C_Enterprise":
		...
		//Connector.Global_settings.Enterprise1CAdress
		req, err := http.NewRequest("DELETE", "http://10.1.99.32/REST_test/hs/exchange/custom_json", nil)
		if err != nil {
			Connector.LoggerConn.ErrorLogger.Println(err.Error())
			return err
		}
